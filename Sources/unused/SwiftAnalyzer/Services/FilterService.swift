...
struct FilterService {
    ...
    func summary(_ items: [ReportItem]) -> (functions: Int, variables: Int, classes: Int, enumCases: Int, protocols: Int, typealiases: Int, parameters: Int, imports: Int) {
        ...
        let typealiases = items.filter { $0.type == .typealias }.count
        let parameters = items.filter { $0.type == .parameter }.count
        let imports = items.filter { $0.type == .import }.count
        return (functions, variables, classes, enumCases, protocols, typealiases, parameters, imports)
    }
}
